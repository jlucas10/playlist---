...
void SystemDriver::createPlaylist(vector<Playlist> &playlists)
{
    ...
    do
    {
        ...
        if (addChoice == 1)
        {
            addSongToPlaylist(playlists.back()); // Add song to the last playlist created
        }
    } while (addChoice != 0);
}

// Function to view all playlists created by the user
void SystemDriver::viewPlaylists(vector<Playlist> &playlists)
{
    ...
    {
        viewSongsInPlaylist(playlists[choice - 1]); // View songs in the selected playlist
    }
    else
    {
        cout << "Invalid choice.\n";
...
// Function to view songs in a playlist
